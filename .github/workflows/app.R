...
tabPanel("User Guide",
         fluidPage(
           ...
           fluidRow(column(width = 12, h2("EDA tab: Shows the interactive graphing function in both dataset 1 (consist of boxplot, bar chart, histogram and density graph) and dataset 2.(consist of multi-line graph)"))),
...
tabPanel("Prediction",
         ...
         sidebarPanel(
           ...
           numericInput("age", label = "Age", value = 30),
           ...
           numericInput("trestbps", label = " Current Resting Blood Pressure", value = 120),
           
           numericInput('chol', label = 'Current Serum Cholesterol(mg/dl)', value = 170),
           ...
           numericInput('thalach', label = 'Maximum heart rate achieved', value = 120),
...
#--------------------------------------------------------EDA df1------------------    
    ...
    output$variable <- renderUI({ 
      ...
      selectInput("variable","Targeted Variable:", var.opts) # update UI 				 
    }) 
    
    output$group <- renderUI({ 
      obj<-df_clev 
      var.opts<-namel(colnames(obj))
      selectInput("group","Group by:", var.opts) # update UI 				 
    }) 
